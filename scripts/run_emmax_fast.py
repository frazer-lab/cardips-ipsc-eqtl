...
def _reml(eigf, remlf, phe, ind, kin, cov=None):
    ...
    if cov:
        c += ' --covf {}'.format(cov)
	# /raid3/projects/CARDIPS/software/EPACTS-3.2.6/bin/pEmmax reml 
        # --phenof /dev/shm/ENSG00000181449.2/ENSG00000181449.2.phe 
        # --covf /dev/shm/ENSG00000181449.2/ENSG00000181449.2.cov 
        # --kinf /raid3/projects/CARDIPS/analysis/cardips-ipsc-eqtl/output/kinship_matrix/wgs.kinf 
        # --indf /dev/shm/ENSG00000181449.2/ENSG00000181449.2.ind 
        # --out-eigf /dev/shm/ENSG00000181449.2/ENSG00000181449.2.eigR 
        # --out-remlf /dev/shm/ENSG00000181449.2/ENSG00000181449.2.reml
...
def run_emmax(gene_id, vcf, regions, phenotypes, ind, kin, tempdir, outdir,
    ...
    while i < max_permut:
        ...
        remlf = os.path.join(tempdir, '{}.reml'.format(gene_id))
        _reml(eigf, remlf, phenof, ind, kin, cov=cov)
        out = os.path.join(outdir, '{}_permutation_{}.tsv'.format(
